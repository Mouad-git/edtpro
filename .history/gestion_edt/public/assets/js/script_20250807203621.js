...
function handleModuleChange(e) {
    ...
    // Gère l'affichage dynamique de l'étoile

    // 1. On parcourt TOUTES les options pour restaurer leur état initial (avec étoile si régional)
    for (const option of select.options) {
        if (!option.value) continue; // On ignore l'option vide

        const isRegional = option.dataset.regional === 'true';
        // On s'assure que le nom du module n'a pas déjà une étoile pour éviter les doublons
        const baseText = option.value; 

        if (isRegional && !option.textContent.startsWith('⭐')) {
    ...
    // 2. On cible l'option actuellement sélectionnée
    const selectedOption = select.options[select.selectedIndex];

    // 3. Si l'option sélectionnée est régionale, ON RETIRE l'étoile de son texte affiché
    if (selectedOption && selectedOption.dataset.regional === 'true') {
        selectedOption.textContent = selectedOption.textContent.replace('⭐ ', '');
    }
    // ======================== FIN DE LA NOUVELLE LOGIQUE ========================


    // Le reste de la fonction gère le style de la cellule (inchangé)
    const cell = select.closest('td');
    if (cell) {
        cell.classList.remove('module-s1', 'module-s2', 'module-annual', 'module-regional');

        if (module) {
            // On utilise "selectedOption" que l'on a déjà
            if (selectedOption) {
 ...
 */
// ASSUREZ-VOUS QUE CETTE FONCTION EST DÉJÀ À JOUR
function updateModuleOptions(formateur, jour, seance, groupe) {
  ...
  modulesEtStatut.forEach((info, moduleName) => {
    ...
    // Cette partie ajoute l'étoile au texte de l'option
    if (info.estRegional) {
...
// --- FIN FONCTION : handleDrop ---
    ...
    appData.formateurs.forEach(formateur => {
        tableHTML += `<tr data-formateur="${formateur}">
            ...
            <td><input type="number" class="formateur-hours-autogen" value="25"></td>
        ...
        for (const pair of pairs) {
            ...
            for (const space of shuffledSpaces) {
                ...
                if (task.type === 'synchrone') {
                    // CORRECTION CLÉ : On vérifie que la limite de sessions TEAMS n'est pas atteinte
...
function tryPlaceSingle(task, slot, newTimetableData, trackers) {
    ...
    task.groupe.trim().split(/\s+/).filter(Boolean).forEach(g => trackers.groupSlots[`${jour}-${seance}-${g}`] = true);
