...
// =================== SOLUTION : AJOUTEZ CE BLOC ===================
if (!function_exists('secure_log')) {
    /**
     * Écrit un message dans un fichier de log de manière sécurisée.
     * @param string $message Le message à logger.
     * @param string $level Le niveau du log (ex: INFO, ERROR, WARNING).
     */
    function secure_log($message, $level = 'INFO') {
        // Chemin vers le fichier de log. Assurez-vous que le dossier 'logs' existe !
        ...
        // Message formaté avec date, heure et niveau
        $formatted_message = date('[Y-m-d H:i:s]') . " [$level] - " . $message . PHP_EOL;
        
        // Utilise error_log() pour écrire dans le fichier de manière atomique et sécurisée
        // Le 3ème paramètre indique à error_log d'écrire dans un fichier spécifié.
