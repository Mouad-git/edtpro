...
try {
    ...
    $stmt = $pdo->prepare("SELECT is_setup_complete FROM utilisateurs WHERE id = ?");
...
$setup_status = $stmt->fetchColumn();
    ...
    if (!$setup_status) {
        // Si la configuration n'est pas terminée, on redirige vers la page de setup
        $_SESSION['utilisateur_id'] = $user['id']; // On ouvre quand même la session
        echo json_encode([
            'success' => true,
            'action' => 'redirect',
            'url' => 'setup.html' // Redirection vers la nouvelle page
        ]);
        exit;
    }
