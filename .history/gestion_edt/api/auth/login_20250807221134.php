 ...
 *    - Si 1, le redirige directement vers l'application (emploi.html).
...
$stmt = $pdo->prepare("SELECT id, mot_de_passe, is_verified, is_setup_complete FROM utilisateurs WHERE email = ?");
    ...
    if (!$user['is_setup_complete']) {
        ...
        // --- LIGNE MANQUANTE ---
        // On doit aussi trouver l'ID de l'établissement et le mettre dans la session !
        $stmt_etab = $pdo->prepare("SELECT id FROM etablissements WHERE utilisateur_id = ?");
