...
try {
    ...
    switch ($newStatus) {
        ...
        case 'blocked':
            ...
            try {
                $colCheck = $pdo->prepare("SELECT COUNT(*) AS cnt FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'utilisateurs' AND COLUMN_NAME = 'date_blocage'");
                $colCheck->execute();
                ...
                // On ne bloque pas l'opération si le check échoue, on continue et laissera l'UPDATE lancer une exception si besoin
            }
            // On met aussi à NULL les autres dates pour cohérence
