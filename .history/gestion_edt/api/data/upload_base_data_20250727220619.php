...
try {
    ...
    foreach ($rows as $rowNumber => $row) {
        ...
        // --- LECTURE DES HEURES S1 & S2 (déjà intégré) ---
        ...
        // --- FIN LECTURE DES HEURES S1 & S2 ---
        
        // --- LECTURE DE L'INFORMATION RÉGIONALE (NOUVEAU) ---
        // Lire la colonne S. Supposons que 'S' est la lettre de la colonne.
        $colonne_s_valeur = trim($row['S'] ?? 'N'); // Valeur de la colonne S, 'N' par défaut si vide
        // Convertir en booléen : 'O'/'o'/'1'/'Y'/'y'/'YES'/'yes'/'TRUE'/'true' = true, tout le reste = false
        $is_regional = in_array(strtoupper($colonne_s_valeur), ['O', '1', 'Y', 'YES', 'TRUE']);
        // --- FIN LECTURE DE L'INFORMATION RÉGIONALE ---
        ...
        // --- INCLUSION DES DONNÉES DANS LES AFFECTATIONS (MODIFIÉ) ---
        // Inclusion de s1_heures, s2_heures et regional dans les affectations
        if ($formateurP && $groupe && $module) {
            $affectations[] = [
                ...
                'regional' => $is_regional   // <-- Ajout de l'information régionale
            ];
        }
    ...
    if(!empty($affectations)) $insertStmt->execute([$etablissement_id, 'affectation', json_encode($affectations)]); // Ce JSON contiendra maintenant s1_heures, s2_heures et regional
