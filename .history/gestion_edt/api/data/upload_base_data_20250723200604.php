...
try {
    ...
    foreach ($rows as $rowNumber => $row) {
        ...
        // La colonne E que vous avez mentionnée (Filière) est implicitement liée
        // au groupe sur la même ligne. Notre logique est donc correcte.

        // =====================================================================
        // == LOGIQUE DU SEMESTRE (Appliquée à CHAQUE ligne) ==
        // =====================================================================
        // On lit les colonnes X et AB pour CETTE LIGNE SPÉCIFIQUE
        $valeurX = (float)($row['X'] ?? 0);
        $valeurAB = (float)($row['AB'] ?? 0);

        // On détermine le semestre pour CETTE LIGNE
        $semestre = 'S2'; // Par défaut
        ...
        // On crée les affectations seulement si un module est présent
        if ($module) {
            // Pour le groupe "présentiel", on lui associe le semestre calculé pour cette ligne
            if ($formateurP && $groupe) {
                $affectations[] = [
                    ...
                    'module' => $module, 
                    'type' => 'presentiel', 
                    'semestre' => $semestre // Le semestre est correctement lié au groupe et au module de cette ligne
                ];
            }
            // Pour le groupe "synchrone", on fait de même
            if ($formateurS && $fusionGroupe) {
                $affectations[] = [
                    ...
                    'module' => $module, 
                    'type' => 'synchrone', 
                    'semestre' => $semestre // Le semestre est aussi correct ici
                ];
            }
        }
    }
