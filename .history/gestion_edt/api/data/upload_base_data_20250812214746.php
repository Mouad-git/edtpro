...
try {
    ...
    // Colonne P (Mode): valeurs "Résidentiel" ou "Alterné"
    $modeIndex = array_search('Mode', $header); // peut être introuvable selon le libellé exact
    ...
    foreach ($dataRows as $row) {
        ...
        // Enregistrer le mode du groupe si disponible (détection robuste)
        if ($groupe) {
            $modeRaw = '';
            if ($modeIndex !== false) {
                $modeRaw = trim((string)($row[$modeIndex] ?? ''));
            } elseif (isset($row['P'])) { // Fallback: colonne P
                $modeRaw = trim((string)($row['P'] ?? ''));
            }
            if ($modeRaw !== '') {
    ...
    // Upsert des modes si on a pu les extraire
    if(!empty($groupeModes)) {
        $pdo->prepare("INSERT INTO donnees_de_base (etablissement_id, type_donnee, donnees_json) VALUES (?, 'groupe_mode', ?) ON DUPLICATE KEY UPDATE donnees_json = VALUES(donnees_json)")
            ->execute([$etablissement_id, json_encode($groupeModes)]);
    }
