...
try {
    ...
    function getFormattedName($name) {
     ...
     // NOUVELLE STRATÉGIE : Variables pour garder en mémoire le contexte actuel
     $filiereActuelle = null;
     $semestreActuel = null;
     ...
     foreach ($rows as $rowNumber => $row) {
         ...
         $module = trim($row['Q'] ?? '');
         $filiereLigne = trim($row['E'] ?? ''); // Lecture de la filière sur la ligne
 
         // --- DÉTECTION DES LIGNES "TITRE" ---
         // On vérifie si la colonne "Module" (Q) contient un de nos mots-clés
         $valeurModule = strtolower($module); // On met en minuscule pour être insensible à la casse
 
         if (strpos($valeurModule, 'semestre 1') !== false || strpos($valeurModule, 's1') !== false) {
             $semestreActuel = 'S1';
             $filiereActuelle = $filiereLigne; // On met à jour la filière en même temps
             continue; // On passe à la ligne suivante, car cette ligne est un titre, pas une affectation
         }
         if (strpos($valeurModule, 'semestre 2') !== false || strpos($valeurModule, 's2') !== false) {
             $semestreActuel = 'S2';
             $filiereActuelle = $filiereLigne;
             continue;
         }
         if (strpos($valeurModule, 'annuel') !== false) {
             $semestreActuel = 'Annuel';
             $filiereActuelle = $filiereLigne;
             continue;
         }
 
         // --- TRAITEMENT D'UNE LIGNE "AFFECTATION" NORMALE ---
 
         // On vérifie qu'on a bien les infos minimales ET qu'on a déjà un semestre en mémoire
         if ($module && $semestreActuel) {
             
             // On s'assure que la filière de la ligne correspond à la filière de la section actuelle
             // pour éviter les erreurs si le fichier est mal formaté.
             if ($filiereLigne === $filiereActuelle) {
                 ...
                 // On utilise la variable $semestreActuel gardée en mémoire
