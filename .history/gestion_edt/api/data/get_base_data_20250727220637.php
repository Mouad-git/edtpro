    ...
    foreach ($baseDataResults as $row) {
        ...
        switch ($row['type_donnee']) {
            ...
            case 'affectation':
                ...
                // les champs s1_heures, s2_heures et regional sont présents et sont du bon type.
                // On suppose que ces champs sont déjà dans le JSON récupéré par upload_base_data.php.
                foreach ($data as &$affectation_item) { // Notez le & pour référence
                    // S'assurer que les clés existent et sont des nombres/booléens, sinon leur donner une valeur par défaut
                    $affectation_item['s1_heures'] = isset($affectation_item['s1_heures']) ? (float)$affectation_item['s1_heures'] : 0;
                    $affectation_item['s2_heures'] = isset($ffectation_item['s2_heures']) ? (float)$affectation_item['s2_heures'] : 0;
                    // S'assurer que 'regional' est un booléen. json_decode le fera si c'était true/false, mais pas si c'était 0/1 ou "true"/"false"
                    // upload_base_data.php stocke un booléen, donc cela devrait être bon. On force quand même pour la robustesse.
                    $affectation_item['regional'] = isset($affectation_item['regional']) ? filter_var($affectation_item['regional'], FILTER_VALIDATE_BOOLEAN) : false;
                }
