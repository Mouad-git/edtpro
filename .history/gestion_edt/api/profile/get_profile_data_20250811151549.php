...
*   **Logique pour les Formateurs :** J'ai adapté votre logique pour les formateurs afin qu'elle corresponde à la structure de table que nous avons réellement construite (`donnees_de_base` avec un champ JSON) au lieu de l'hypothétique `formateurs_details`.
*   **Logique pour le Calendrier :** De même, j'ai adapté la logique pour lire depuis `donnees_de_base` au lieu de l'hypothétique table `calendrier`.
*   **Ajout des Groupes :** Le bloc de code le plus important que j'ai ajouté est celui qui va chercher la liste des groupes. Il fonctionne exactement comme celui pour les formateurs, mais il cherche `type_donnee = 'groupe'`.

Avec ce script corrigé et complet, votre page `profile.html` recevra maintenant toutes les informations dont elle a besoin, y compris la liste des groupes pour l'onglet "Stages", et le problème sera résolu.