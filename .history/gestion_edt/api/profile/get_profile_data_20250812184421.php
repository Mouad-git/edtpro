...
try {
    ...
    // Fallback si pas encore stocké: déduire depuis la dernière entrée de `donnees_avancement`
    if (!$groupModes || (is_array($groupModes) && count($groupModes) === 0)) {
        $stmtAva = $pdo->prepare("SELECT donnees_json FROM donnees_avancement WHERE etablissement_id = ? ORDER BY id DESC LIMIT 1");
