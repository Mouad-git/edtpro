...
// Les données importées stockent les groupes "présentiel" sous type_donnee = 'groupe'
// et les groupes de fusion (alterné/Teams) sous type_donnee = 'fusion_groupe'.
// Le frontend attend des objets { nom } dans deux tableaux: Residentiel et Alterne.

// Groupes résidentiels
$stmtGroupes = $pdo->prepare(
    "SELECT donnees_json FROM donnees_de_base WHERE etablissement_id = ? AND type_donnee = 'groupe'"
);
$stmtGroupes->execute([$etablissement_id]);
$groupesJson = $stmtGroupes->fetchColumn();
$groupesResidentielRaw = $groupesJson ? json_decode($groupesJson, true) : [];

// Groupes alternés (fusions)
$stmtFusion = $pdo->prepare(
    "SELECT donnees_json FROM donnees_de_base WHERE etablissement_id = ? AND type_donnee = 'fusion_groupe'"
);
$stmtFusion->execute([$etablissement_id]);
$fusionJson = $stmtFusion->fetchColumn();
$groupesAlterneRaw = $fusionJson ? json_decode($fusionJson, true) : [];

// Normalisation en objets { nom }
$normalizeGroupList = function ($items) {
    $result = [];
    if (is_array($items)) {
        foreach ($items as $item) {
            if (is_string($item)) {
                $result[] = ['nom' => $item];
            } elseif (is_array($item) && isset($item['nom'])) {
                $result[] = ['nom' => $item['nom']];
            } else {
                // Valeur inattendue: on tente un cast simple pour éviter "undefined" côté frontend
                $result[] = ['nom' => (string)($item ?? '')];
            }
        }
    }
    return $result;
...
$data['groupes_classes'] = [
    'Residentiel' => $normalizeGroupList($groupesResidentielRaw),
    'Alterne'     => $normalizeGroupList($groupesAlterneRaw),
];
