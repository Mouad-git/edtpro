...
$nomFormateurSyncIndex = 22; // Colonne W
   ...
   // --- 6. Marquer la configuration comme terminée ET FINALISER LA SESSION ---
    
    // a) Mettre à jour la base de données (inchangé)
    $stmtSetup = $pdo->prepare("UPDATE utilisateurs SET is_setup_complete = TRUE WHERE id = ?");
    $stmtSetup->execute([$utilisateur_id]);
    
    // b) --- LA CORRECTION CRUCIALE EST ICI ---
    // On met à jour la session PHP en cours pour la rendre "complète".
    // Maintenant, quand l'utilisateur arrivera sur admin.html, le gardien strict sera satisfait.
    $_SESSION['etablissement_id'] = $etablissement_id;
    // ------------------------------------------

    // Si on arrive ici, toutes les opérations BDD sont prêtes à être validées.
    $pdo->commit();
