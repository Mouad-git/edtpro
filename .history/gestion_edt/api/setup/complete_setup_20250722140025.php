...
$nomFormateurSyncIndex = 22; // Colonne W
    ...
    // ÉTAPE 6 : Marquer la configuration comme terminée (inchangé)
    $stmtSetup = $pdo->prepare("UPDATE utilisateurs SET is_setup_complete = TRUE WHERE id = ?");
    $stmtSetup->execute([$utilisateur_id]);

    // --- CORRECTION CRUCIALE : MISE À JOUR DE LA SESSION ---
    // Maintenant que la configuration est terminée, on peut ajouter l'ID de l'établissement
    // à la session pour que l'utilisateur soit pleinement authentifié pour le reste de l'application.
    // L'ID $etablissement_id est déjà disponible au début de notre script.
