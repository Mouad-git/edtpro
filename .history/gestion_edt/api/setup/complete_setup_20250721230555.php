...
if (empty($rawExcelData) || !isset($rawExcelData[0])) {
    throw new Exception("Les données Excel reçues sont vides ou mal formatées.");
}
...
$header = array_map('trim', $rawExcelData[0]);
// On prend le reste comme les lignes de données
$dataRows = array_slice($rawExcelData, 1);
    
    // On cherche les index des colonnes dynamiquement
    $nomFormateurIndex = array_search('Formateur Affecté Présentiel Actif', $header);
    $nomFormateurSyncIndex = array_search('Formateur Affecté Syn Actif', $header); // Assurez-vous que ce titre est correct
    ...
    // Si une colonne cruciale est manquante, on arrête
    if ($groupeIndex === false || $moduleIndex === false) {
        throw new Exception("Les colonnes 'Groupe' ou 'Module' sont introuvables dans l'en-tête du fichier Excel.");
    }
